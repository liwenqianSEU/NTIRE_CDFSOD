...
class QueryPrototypeTransformer(nn.Module):
    ...
    def __init__(self, feature_dim=1024, ratio=0.5):
        ...
        # Transformer Cross-Attention Layer (Query=F_q, Key/Value=F_ins)
        # self.cross_attention = nn.MultiheadAttention(embed_dim=feature_dim, num_heads=num_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, F_ins, F_q):
        """
        Args:
            F_q: Query feature map, shape (batch, 1024, 58, 58)
            F_ins: Support instance features, shape (7, kshot, 1024)

        Returns:
            F_prototypes: Output prototypes, shape (7, 1024)
        """
        batch_size = F_q.shape[0] 
        
        # Step 1: 对 Query 进行全局平均池化
        F_q_pool = F.adaptive_avg_pool2d(F_q, (1, 1)).squeeze(-1).squeeze(-1)  # (batch, 1024)
        
        # Step 2: 对 Support 改变形状
        F_ins_fl = F_ins.reshape(-1, F_ins.shape[-1])  # (7*kshot, 1024)
        
        # Step 3: 让 Query 参与特征计算（Cross Attention）
        Q = self.to_q(F_q_pool)  # (batch, 1024) 作为 Query
        K = self.to_k(F_ins_fl)  # (7*kshot, 1024) 作为 Key
        V = self.to_v(F_ins_fl)  # (7*kshot, 1024) 作为 Value
        
        # 计算注意力权重
        attn_scores = torch.matmul(Q, K.transpose(0, 1)) / math.sqrt(Q.shape[-1])  # (batch, 7*kshot)
        attn_weights = F.softmax(attn_scores, dim=-1)  # (batch, 7*kshot)
        
        # 计算加权特征
        F_q_attended = torch.matmul(attn_weights, V)  # (batch, 1024)
        # F_q_attended = self.final_fc(F_q_attended)  # (batch, 1024)
        
        # condition (batch, 7*kshot)
        F_q_norm = F_q_pool / (F_q_pool.norm(dim=-1, keepdim=True) + 1e-6)  # 归一化
        F_ins_norm = F_ins_fl / (F_ins_fl.norm(dim=-1, keepdim=True) + 1e-6)  # 归一化
        condition = F.cosine_similarity(F_ins_norm.unsqueeze(1), F_q_norm.unsqueeze(0),dim=-1)
        
        F_ins_transformed = F_ins_fl + torch.matmul(condition, F_q_attended)   # (7*kshot, 1024)
        
        F_ins_aug = F_ins_transformed.reshape(F_ins.shape)  # (7, kshot, 1024)
        
        F_ins_prototype = F_ins_aug.mean(dim=1)  # (7, 1024)
        
        # Step 4: 计算最终 Prototypes sim: 7*kshot, 7
        sim = F.cosine_similarity(F_ins_transformed.unsqueeze(1), F_ins_prototype.unsqueeze(0), dim=-1)
        sim_soft = F.softmax(sim, dim=-1)
        F_ins_transform_prototype = torch.matmul(sim_soft, F_ins_prototype).view(F_ins.shape).mean(dim=1)  # (7, kshot, 1024)
        F_ins_prototype_new = self.ratio * F_ins_prototype + (1 - self.ratio) * F_ins_transform_prototype
        
        return F_ins_prototype_new
        
        # batch_size = F_q.shape[0]  # 获取 batch 维度
        
        # # Step 1: 对 Query 进行全局平均池化
        # F_q_pool = F.adaptive_avg_pool2d(F_q, (1, 1)).squeeze(-1).squeeze(-1)  # (batch, 1024)
        
        # # Step 2: 对 Support 进行均值池化
        # F_ins_avg = F_ins.mean(dim=1)  # (7, 1024)  对 k-shot 取均值
        # F_ins_expanded = F_ins_avg.unsqueeze(0).expand(batch_size, -1, -1)  # (batch, 7, 1024)

        # # Step 3: 让 Query 参与特征计算（Cross Attention）
        # F_q_expanded = F_q_pool.unsqueeze(1)  # (batch, 1, 1024) 作为 Query
        # F_q_attended, _ = self.cross_attention(F_q_expanded, F_ins_expanded, F_ins_expanded)  # (batch, 1, 1024)
        
        # # Step 4: 让 Support 经过 Transformer 进行特征增强
        # F_ins_transformed = self.transformer(F_ins_expanded)  # (batch, 7, 1024)
        
        # # Step 5: 计算最终 Prototypes，Query-Attended + Transformed Support
        # F_prototypes = (F_q_attended.mean(dim=0) + F_ins_transformed.mean(dim=0)) / 2  # (7, 1024)
