...
class QueryPrototypeTransformer(nn.Module):
    # def __init__(self, feature_dim=1024, num_heads=8, num_layers=1, ff_dim=2048, dropout=0.1):
    def __init__(self, feature_dim=1024, ratio=0.5):
        super(QueryPrototypeTransformer, self).__init__()

        self.to_q = nn.Linear(feature_dim, feature_dim)
        self.to_k1 = nn.Linear(feature_dim, feature_dim)
        self.to_k2 = nn.Linear(feature_dim, feature_dim)
        self.to_v = nn.Linear(feature_dim, feature_dim)
        self.final_fc = nn.Linear(feature_dim, feature_dim)
        self.sigmoid = nn.Sigmoid()
        self.attn_drop = nn.Dropout(0.1)
        self.query_proj = nn.Linear(feature_dim, feature_dim)
        self.query_proj_drop = nn.Dropout(0.1)
        self.softmax = nn.Softmax(dim=-1)
        self.ratio = ratio
                
    def forward(self, F_s, F_q):
        """
        Args:
            F_q: Query feature map, shape (batch, 1024, 58, 58)
            F_s: Support instance features, shape (7, kshot, 1024)

        Returns:
            F_prototypes: Output prototypes, shape (7, 1024)
        """
        K_shot = F_s.shape[1]
        
        ori_prototypes = F_s.mean(dim=1)  # (7, 1024)
        
        # Step 1: 对 Query 进行全局平均池化
        F_q_pool = F.adaptive_avg_pool2d(F_q, (1, 1)).squeeze(-1).squeeze(-1)  # (batch, 1024)
        
        # Step 2: 对 Support 改变形状
        F_s_reshape = F_s.reshape(-1, F_s.shape[-1])  # (7*kshot, 1024)
        
        F_cat = torch.cat([F_s_reshape, F_q_pool], dim=0)  # (7*kshot+batch, 1024)
        
        # Step 3: 让 Query 参与特征计算（Cross Attention）
        Q = self.to_q(F_q_pool)  # (batch, 1024) 作为 Query
        K1 = self.to_k1(F_q_pool)  # (7*kshot, 1024) 作为 Key
        K2 = self.to_k2(F_s_reshape)  # (7*kshot, 1024) 作为 Key
        V = self.to_v(F_cat)  # (7*kshot, 1024) 作为 Value
        
        # 计算自注意力和注意力权重
        cos_eps = 1e-7
        self_attn_score = torch.matmul(Q, K1.transpose(0, 1)) / math.sqrt(Q.shape[-1])  # (batch, batchD)
        Q_norm = F.normalize(Q, dim=-1)
        K2_norm = F.normalize(K2, dim=-1)
        cross_attn_score = torch.matmul(Q, K2.transpose(0, 1)) / (torch.matmul(Q_norm, K2_norm.transpose(0, 1)) + cos_eps)  # (batch, 7*kshot)
        
        attn_score = torch.cat([self_attn_score, cross_attn_score], dim=-1)  # (batch, 7*kshot+batch)
        attn_weights = self.softmax(attn_score)  # (batch, 7*kshot+batch)
        attn_weights = self.attn_drop(attn_weights)
        
        # 计算加权特征
        F_q_hat = F_q_pool + torch.matmul(attn_weights, V)  # (batch, 1024)
        
        F_q_hat = self.query_proj(F_q_hat)
        F_q_hat = self.query_proj_drop(F_q_hat)
        
        #计算F_q和F_s的相似度, (7*kshot, batch)
        cos_simarity = F.cosine_similarity(F_s_reshape.unsqueeze(1), F_q_pool.unsqueeze(0), dim=-1)  # (7*kshot, batch)
        F_s_hat = F_s_reshape + torch.matmul(self.softmax(cos_simarity), F_q_hat)  # (7*kshot, 1024)
        
        # 3.18 删除这几行
        weight = self.final_fc(F_s_hat)
        weight = self.sigmoid(weight)
        F_s_hat = F_s_hat * weight
        F_s_prototypes = F_s_hat.reshape(-1, K_shot, F_s.shape[-1]).mean(dim=1)
        
        out_prototypes = self.ratio * F.normalize(F_s_prototypes, dim=-1) + (1 - self.ratio) * F.normalize(ori_prototypes, dim=-1)
...
class OpenSetDetectorWithExamples(nn.Module):
    ...
    def __init__(self,
        ...
        # elif self.set_ours and self.num_train_classes != 1:
        #     self.num_domain_prompter = int(self.num_train_classes * 2)
        #     self.domain_prompter = nn.Embedding(num_embeddings=self.num_domain_prompter, embedding_dim=self.ndim).cuda()
        #     self.domain_prompter_info_nce = InfoNCE()
        #     self.prototype_classifier = LinearClassifier(self.ndim, self.num_train_classes, add_mlp=False, drop=0.)
        ...
        if len(prototypes.shape) == 3:
            if self.set_cd_vito:
                ...
                self.train_class_weight = None
                self.register_buffer("test_class_weight", F.normalize(prototypes.mean(dim=1)[torch.as_tensor(test_class_order)], dim=-1))
                self.adapter = QueryPrototypeTransformer(feature_dim=prototypes.shape[-1], ratio=attn_fuse_ratio)
            else:
        ...
        if not self.set_cd_vito and not self.set_ours:
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        loss_dict = {}
        total = self.class_weights.weight.shape[0]
        num_classes = total // self.k_shot
        ...
        # roi_features = nn.Parameter(roi_features)
        ...
        if self.set_cd_vito:
            ...
            # roi_features
            self.train_class_weight = self.adapter(self.class_weights.weight.view(-1, self.k_shot, self.ndim), patch_tokens)
            self.update_test_buffer(self.train_class_weight)
        if self.training:
        ...
        if (self.training and (not self.only_train_mask)) or (not self.training):
            ...
            if self.set_cd_vito:
                ...
                bg_feats = roi_features.transpose(-2, -1) @ self.bg_tokens.weight.t()
            else:
        ...
        if self.training:
            ...
            # elif self.set_ours and self.num_train_classes != 1:
                ...
                # proto_contrastive_loss = self.proto_info_nce(query, positive_keys, negative_keys)
                # loss_dict['proto_contrastive_loss'] = proto_contrastive_loss
                # similarity_matrix = F.cosine_similarity(class_prototypes.unsqueeze(1), class_prototypes.unsqueeze(0), dim=-1)  # (num_classes_proto, num_classes_proto)

                # num_classes_proto = class_prototypes.size(0)
                
                # # 生成正样本对（对角线）和负样本对
                # mask = torch.eye(num_classes_proto, dtype=torch.bool, device=class_prototypes.device)  # (num_classes, num_classes)

                # positives = similarity_matrix[mask].unsqueeze(1)  # (num_classes, 1)
                # negatives = similarity_matrix[~mask].view(num_classes_proto, -1)  # (num_classes, num_classes-1)

                # # 计算 InfoNCE Loss
                # self.temperature = 0.1
                # logits = torch.cat([positives, negatives], dim=1) / self.temperature  # (num_classes, num_classes)
                # labels = torch.zeros(num_classes_proto, dtype=torch.long, device=class_prototypes.device)  # 正样本索引都是0
                # loss_dict['proto_contrastive_loss'] = F.cross_entropy(logits, labels)
