...
class CLIPFastRCNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        """
        super().__init__()
        self.offline_backbone = offline_backbone # ResNet
        self.backbone = backbone # Modify ResNet
        self.lang_encoder = language_encoder # None, 似乎没有用上
    ...
    def from_config(cls, cfg):
        ...
        return {
            "backbone_fp16": cfg.SOLVER.AMP.ENABLED,
            ...
            # ! these two will automatically set false for my modules
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        with torch.no_grad():  
            if self.clip_crop_region_type == "GT":  # from ground-truth
                ...
                images = self.offline_preprocess_image(batched_inputs) # [16, 3, 800, 1333]
                features = self.offline_backbone(images.tensor) # ResNet, 是 RPN, {res4: [16, 1024, 50, 84]}
                if self.offline_proposal_generator is not None:
                    proposals, _ = self.offline_proposal_generator(images, features, None)  # proposal -> bunch of boxes, 所以这个就是单独训了一个 RPN！ 所以慢!
        ...
        if self.backbone_fp16:
            ...
            features = self.backbone(images.tensor) # modify resnet, why it is different than previous? 是 CLIP
    ...
    def inference(
        ...
        if self.use_clip_c4: # use C4 + resnet weights from CLIP
            if self.use_clip_attpool: # use att_pool from CLIP to match dimension
                results, _ = self.roi_heads(images, features, proposals, gt_instances, res5=self.backbone.layer4, attnpool=self.backbone.attnpool)
            else: # use mean pool
        ...
        if do_postprocess:
            ...
            r = CLIPFastRCNN._postprocess(results, batched_inputs)
            return r
        else:
            return results
